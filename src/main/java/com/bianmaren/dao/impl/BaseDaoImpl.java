...
import com.bianmaren.Filter.Operator;
import com.bianmaren.Order;
import com.bianmaren.Order.Direction;
import com.bianmaren.Page;
import com.bianmaren.Pageable;
import com.bianmaren.dao.BaseDao;
import com.bianmaren.entity.OrderEntity;
...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	public Page<T> findPage(Pageable pageable,List<Filter> filters,List<Order> orders) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(entityClass);

		criteriaQuery.select(criteriaQuery.from(entityClass));
		return findPage(criteriaQuery, pageable,filters,orders);
	}

	protected Page<T> findPage(CriteriaQuery<T> criteriaQuery, Pageable pageable,List<Filter>filters,List<Order> orders) {
	...
	private synchronized String getAlias(Selection<?> selection) {
		if (selection != null) {
			...
			if (alias == null) {
				...
				alias = "bianmarenGeneratedAlias" + aliasCount++;
